...
!! 1) act에서 계좌번호 - 고객번호 2)cus에서 고객번호 - 연령대 3) 연령대를 trd_kr에 갖다붙이기
...
labels = [ ORR_MDI[j-1] for j in idx]
Y_means = [Y['주문매체구분코드'].value_counts()[i] for i in idx]
Z_means = [Z['주문매체구분코드'].value_counts()[i] for i in idx]

x = np.arange(len(labels))  # the label locations
width = 0.35  # the width of the bars

fig, ax = plt.subplots(figsize=(20,10))
rects1 = ax.bar(x - width/2, Y_means, width, color='#8fd99f', label='Y')
rects2 = ax.bar(x + width/2, Z_means, width, color='#8faad9', label='Z')

# Add some text for labels, title and custom x-axis tick labels, etc.
ax.set_ylabel('Counts')
ax.set_title('Order Media of Y & Z Generation')
ax.set_xticks(x)
ax.set_xticklabels(labels)
ax.legend()
...
buy_foreign_y['고객번호'].value_counts()

"""Y 세대는 382명이 해외 주식 매입에 참여했음을 알 수 있습니다."""

buy_foreign_z['고객번호'].value_counts()

"""Z 세대는 96명이 해외 주식 매입에 참여했음을 알 수 있습니다."""

grouped_y = buy_foreign_y[['주문날짜', '체결수량']].groupby('주문날짜')
grouped_z = buy_foreign_z[['주문날짜', '체결수량']].groupby('주문날짜')

summed_y = grouped_y.sum()
summed_z = grouped_z.sum()

"""이미 여기서 주문날짜 일수부터 100일 넘게 차이남."""

summed_y.reset_index(inplace=True)
summed_z.reset_index(inplace=True)
...
chart_studio.tools.set_credentials_file(username='hewas1230', api_key='R63b8wTSm5RRhOVLZoUA')

fig = px.line(summed_z, x="주문날짜", y="체결수량", title='Life expectancy in Korea, Rep.')
fig.add_trace(go.Line( x=summed_y["주문날짜"], y=summed_y["체결수량"], name="daily"))
fig.show()
#py.iplot(fig)
...
fig.add_trace(go.Line( x=summed_y["주문날짜"], y=summed_y["체결수량"], name="Y"), 1, 1)

fig.add_trace(go.Line( x=summed_z["주문날짜"], y=summed_z["체결수량"], name="Z"), 1, 1)
